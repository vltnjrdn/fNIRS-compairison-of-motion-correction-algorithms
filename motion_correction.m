...
% 3. the RLOESS.mat file (the motion correction algorithm takes too long)
...
%% 2. convert intensity to optical density and extract experiment data
...
%% 3. motion artifact correction 
% (takes abt 2 min, RLOESS takes too long, I saved the dataset as RLOESS.mat)
...
for idx = 1:length(dod_list)
    ...
    n_all = sum(tIncCh(:) == 0); % number
    r_all = n_all / (numel(tIncCh)); % ratio
...
for idx = 1:length(dc_list)
    HbO_list{idx} = squeeze(dc_list{idx}(:, 1, :));  % time x chromophores(3) x channels(28)
end

% Extract desoxygenated Haemoglobin
HbR_list = cell(size(dc_list));
for idx = 1:length(dc_list)
    HbR_list{idx} = squeeze(dc_list{idx}(:, 2, :));  % time x chromophores(3) x channels (28)
end
...
channel_to_plot = 20; % choose a channel between 1 and 28
